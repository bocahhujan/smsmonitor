...
class TerminalWidget(QWidget):
    ...
    def __init__(self, mysqlcoonect , parent=None):
        ...
        self.mysqlcoonect = mysqlcoonect
        #self.smslog = smslog
    ...
    def send_command(self, command):
        ...
        #self.command_txt.insert(command)
    ...
    def readSms(self,text):
        """
        Memecat text dan membaca sms
        """
        tarr = text.split(',');
        self.idsms = tarr[-1];
        #print idsms;
        self.send_command('AT+CMGR='+self.idsms)


    def smsMonitor(self , text):
        """
        Monitor sms lewat sms log
        """
        st = '+CMTI: "SM",'
        if st in text:
            #print text;
            ...
            self.readSmsMonitor(text);

    def readSmsMonitor(self , text):
        st = '+CMGR:';
        if st in text :
            #print text ;
            tarr1 = text.split('\n');
            #print tarr1
            tarr = tarr1[1].split(',');
            no = str(tarr[1]).replace('"','')
            tgl = str(tarr[3]).replace('"','')
            tgl = datetime.strptime(tgl , "%y/%m/%d")
            tgl = tgl.strftime("%Y-%m-%d");
            jam = str(tarr[4]).replace('"','')
            jam = jam.split('+');
            jam = jam[0]
            isi = tarr1[2]
            qry = "insert into sms(nomer , tanggal , jam , pesan ) value ( '"+no+"' , '"+tgl+"' , '"+jam+"' , '"+isi+"') "
            #print qry
            self.mysqlcoonect.insert(qry)
            #self.smslog.additemRow(no , tgl , jam , isi)
            self.updateadd.emit(no , tgl , jam ,  isi )
            self.send_command('AT+CMGD='+self.idsms)
            #lambda: mysqlcoonect.insert(qry)
            #self.insertDb(qry);
